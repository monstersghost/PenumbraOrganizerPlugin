...
- **`Refreshing` runs exactly once after every operation that reached execution, unless the terminal fault establishes that the provider cannot safely or meaningfully service the refresh.** `ProviderUnavailable` integrity failures settle directly without entering `Refreshing` (the adapter itself is judged unusable, so attempting a refresh call would very likely also fail and strand the operation in `RequiresRecovery` for no benefit). Every other conclusion — finished, cancelled, or an `UnexpectedFatalException` integrity failure — still runs `Refreshing` once before settling. (Revised during Task 6's review: the original wording claimed no exception existed; implementation and this text now agree.)
