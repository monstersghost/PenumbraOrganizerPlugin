...
**Gear-shape matching must be a catch-all, not a strict "zero parentheses" rule.** The `Moonward Samurai
Blade (Sheathe)` example above already establishes that an item name plus a parenthetical qualifier is
still Gear shape, not a separate suffix shape. A third spike dump (237 mods / 1,622 keys, a fresh
library, 2026-07-14) confirmed this generalizes further: ~110 item keys carry a dye/tier/color-variant
suffix (`Dated Canvas Bottom (Auburn)`, `Promise of Devotion (Platinum)`, `Promise of Innocence
(Standard)`, etc.), and one carries an unrelated qualifier (`Capella (Card Holder)`, a Triple Triad card
holder). Every one of these co-occurred with a plain unsuffixed sibling key in the same mod in this
sample, so "Gear wins" already produced the correct classification regardless — but Layer 1's Gear-shape
match needs to be implemented as "any key not matching one of the other reserved shapes" rather than
"an item name with no trailing `(...)` at all." A stricter implementation would risk misclassifying a
hypothetical mod whose *entire* key set is suffixed variants with no unsuffixed sibling (not observed in
any sample so far, but not structurally impossible either).
...
each rule in the priority order above, using real key strings captured in this spec and the spike
dumps, including the specific compilation-pack cases (`Carlotta's Outfit`-shaped: Gear + incidental
Mount key), pure single-purpose cases (`Statice Flight`-shaped: no Gear, only Mount + literal
category words), and a mod with zero changed-item keys at all (`higanbana [bibo]`-shaped, confirmed to
occur in real data by the third spike dump) — it must classify as `Unknown` via the same no-match
fallback as an unrecognized key, not a special-cased empty check. The `ByModType` grouping logic gets
the same kind of test `ByCreator`'s grouping already has. The real bulk IPC call and its data shape are
only verifiable in-game, same as the rest of this plugin's IPC surface.
...
2. `Icon:` keys are preserved but unused — if a mod ever produces one with *no* other recognized key,
   it falls through to `Unknown` untested (not observed in any of the three samples now gathered;
   every real occurrence co-occurred with a decisive key).
...
4. Sample size, while large (~2,270 mods across the two original English-client dumps, cross-checked
   against German- and Japanese-client dumps of the same ~2,035-mod library for localization, plus a
   third fresh-library dump of 237 mods / 1,622 keys gathered 2026-07-14 that confirmed the design
   without surfacing any new top-level shape or `Customization` body-part token), is still drawn from a
